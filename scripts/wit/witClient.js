...
WitClient.prototype.resolveTextCommand = function(textCommand) {
  var requestDTO = {
    "api": "message",
    "version": "v=20141022",
    "method": "POST",
  ...
  requestDTO.url = config.witApiUrl + requestDTO.api + "?" +  requestDTO.version + '&q=' + encodeURIComponent(textCommand);
  //requestDTO.url = "https://api.wit.ai/message?v=20141022&q=what%20is%20the%20temperature";
...
WitClient.prototype.converse = function(userText) {
    var requestDTO = {
    "api": "converse",
    "version": "v=20160330",
    "method": "POST",
  ...
  requestDTO.url = config.witApiUrl + requestDTO.api + "?" +  requestDTO.version + '&session_id=123abc' + '&q=' + encodeURIComponent(userText);
  requestDTO.headers["Authorization"] = "Bearer " + config.serverAccessToken;
  var response = http.request(requestDTO);
  if (response.metadata && response.metadata.status == "failure") {
